...
**From source, if you have the kernel tree:** [`kernel/rk3588s-radxa-cm5-uconsole-aio.dts`](../kernel/rk3588s-radxa-cm5-uconsole-aio.dts) includes the base DTS and adds the AIO's two nodes: UART2 on `uart2m0` for the GPS and I2C7 on `i2c7m2` with the RTC child. SPI4 is deliberately left disabled, see [Other boards in the slot](#other-boards-in-the-slot). Everything else it carries comes from three includes that have nothing to do with the AIO and are useful on their own: `-mainboard.dtsi` (OCP8178 backlight, amplifier enable held low, UART4 console off with `stdout-path` removed), `-hg-fan.dtsi` (the adapter's fan header) and `-battery-10ah.dtsi` (labels for this fork's cell; per machine). Copy the four files next to the base DTS and add the Makefile line (README, step 2), then:
...
**The adapter's fan header**, from the same drawing and measured on 2026-09-25 with a Raspberry Pi PWM fan: the header's PWM goes to the Radxa module's third connector, pin 18, and its tacho to pin 38. On the Radxa CM5 those are GPIO3_D5, hardware **PWM11** in its `m3` pinmux, and GPIO4_A4; the fan follows the PWM line, and the tacho delivers two pulses per revolution once the pad has a pull-up (about 8 700 rpm at full speed). [`kernel/rk3588s-radxa-cm5-uconsole-hg-fan.dtsi`](../kernel/rk3588s-radxa-cm5-uconsole-hg-fan.dtsi) turns that into a `pwm-fan` with a thermal map on the SoC's package temperature: off below 50 °C, three steps above. Both AIO device trees include it; it is adapter hardware and the base DTS can include it the same way. With it the fan shows up as `/sys/class/hwmon/hwmon*/fan1_input` for the speed and as a cooling device under `/sys/class/thermal/`. Without it a Pi fan runs at full speed, because its PWM input floats high. The include also enables the SoC's temperature sensor, `tsadc`, which Radxa's CM5 include and the upstream uConsole DTS leave disabled: before this there were no thermal zones on the machine at all, so neither the fan maps nor the rk3588 tree's CPU throttling and critical trips could work. The sensor driver has to be present early, hence `CONFIG_ROCKCHIP_THERMAL=y` in the build script's fragment; on a kernel where it is a module, `rockchip_thermal` must be loaded before the zones appear.
